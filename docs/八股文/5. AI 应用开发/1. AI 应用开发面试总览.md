...
graph TD
    ...
    subgraph Spring AI Advisor 责任链拦截调度管道
        DS --> IA[2. IntentRecognitionAdvisor <br> 意图高精度二度识别]:::advisor
        IA --> FA[3. FaqSemanticCacheAdvisor <br> FAQ 语义缓存短路拦截]:::advisor
        FA --> UA[4. UserContextAdvisor <br> 自适应画像注入+两级工具计算]:::advisor
        UA --> MA[5. MessageChatMemoryAdvisor <br> Redis 滑动窗口会话记忆加载]:::advisor
        MA --> RA[6. RagAdvisor <br> 条件式 RAG 向量知识检索]:::advisor
        RA --> TA[7. ToolFilterAdvisor <br> 动态工具过滤与注入隔离]:::advisor
        TA --> SA[8. SafeToolCallAdvisor <br> 工具签名去重 + 4轮死循环熔断]:::advisor
    end

    SA --> LLM[9. 大模型调用 / Function Calling]:::component
