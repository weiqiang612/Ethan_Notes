...
graph TD
    ...
    UserRequest[1. 用户请求] --> DS[Dispatcher / ChatClient]
    
    subgraph Spring AI Advisor 链式调度拦截管道
        DS --> IA[2. IntentRecognitionAdvisor <br> 12类意图高精度识别]:::advisor
        IA --> UA[3. UserContextAdvisor <br> 6维记忆持久化 + 动态工具白名单]:::advisor
        UA --> RA[4. RagAdvisor <br> FAQ 知识库混合检索增强]:::advisor
        RA --> TA[5. ToolFilterAdvisor <br> 动态工具过滤与注入隔离]:::advisor
        TA --> SA[6. SafeToolCallAdvisor <br> 工具死循环熔断 + 签名审计]:::advisor
    end

    SA --> LLM[7. 大模型调用 / Function Calling]:::component
    LLM --> Database[(MySQL / Pgvector / Redis)]:::component
```
...
*   **What（sky-ai 项目的 5 大 Advisor 核心职责与执行顺序）**
    1.  `IntentRecognitionAdvisor`（意图识别）：通过轻量 Prompt 精确判定 12 种客服意图，挂载最高优先级。
    2.  `UserContextAdvisor`（画像注入）：根据意图，动态决定 6 维记忆的注入粒度。
    3.  `RagAdvisor`（知识库增强）：针对 FAQ 意图，自动并发召回向量库数据。
    4.  `ToolFilterAdvisor`（工具白名单）：为大模型提供物理级别的工具调用隔离。
    5.  `SafeToolCallAdvisor`（死循环熔断）：锁定在最低优先级，充当底层的安全防爆盾。
*   **How（如何在工程中优雅地链式装配它们？）**
    *   通过 `ChatClient.builder` 动态链式 `advisors()` 装配，结合 Order 优先级机制，使得每个请求自动、无感地划过这 5 道关卡，开发体验极其干净。
...
| :--- | :--- | :--- | :--- |
| **大模型基础与 API 工程** | **[2. 大模型基础与 API 工程.md](file:///c:/Users/Ethan/Desktop/笔记/docs/八股文/5. AI 应用开发/2. 大模型基础与 API 工程.md)** | BPE 分词、Token 成本计算、上下文 Lost in the Middle 规避、采样参数调优、流式 SSE 打字机流转、API 熔断降级 | ⭐️⭐️⭐️ |
| **AI Agent 与工具调用** | **[3. AI Agent 与工具调用.md](file:///c:/Users/Ethan/Desktop/笔记/docs/八股文/5. AI 应用开发/3. AI Agent 与工具调用.md)** | Agent Loop 状态机、Memory 6维持久化、Context 净化、MCP Server 安全治理、Harness 架构、Tool Pinned 避坑 | ⭐️⭐️⭐️⭐️⭐️ (重中之重) |
| **RAG 检索增强生成** | **[4. RAG 检索增强生成.md](file:///c:/Users/Ethan/Desktop/笔记/docs/八股文/5. AI 应用开发/4. RAG 检索增强生成.md)** | 向量检索数学原理、Chunk 动态切分、Embedding 噪点排除、RRF 混合检索、Rerank 二次精排、GraphRAG 知识图谱 | ⭐️⭐️⭐️⭐️ |
| **AI 系统设计与大厂实践** | **[5. AI 系统设计.md](file:///c:/Users/Ethan/Desktop/笔记/docs/八股文/5. AI 应用开发/5. AI 系统设计.md)** | 模型网关架构、多路 Fallback 降级、可观测与 Trace 链条、Golden Set 自动评测、LLM-as-Judge 安全红线治理 | ⭐️⭐️⭐️⭐️ |
